...
function findEmbeddedWithoutIndex(resourceArray, parsedKey) {
  // client did not specify an array index, return full array
  if (resourceArray.length > 1) {
    log.warn('Found HAL embedded resource array with more than one element ' +
        ' for key ' + parsedKey.key + ', choosing the full array.')
  }
  return { doc: resourceArray.map(function(resource) { return resource.original() }) }
}
